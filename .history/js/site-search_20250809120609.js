...
class SiteSearch {
        ...
        if (results.length === 0) {
            searchResults.innerHTML = `
                <div style="padding: 20px; color: #666; text-align: center;">
                    ...
                    <div style="font-size: 14px;">Try searching for:</div>
                    <div style="font-size: 12px; margin-top: 5px; color: #999;">
                        research, people, publications, gallery, news, teaching, contact
                    </div>
                </div>
            `;
            return;
        ...
        const resultsHTML = results.map((result, index) => {
            ...
            return `
                <div class="search-result-item" style="
                    padding: 15px 20px;
                    border-bottom: ${index === results.length - 1 ? 'none' : '1px solid #f0f0f0'};
                    cursor: pointer;
                ...
                " onmouseover="this.style.backgroundColor='#f8f9fa'" 
                   ...
                   onclick="window.siteSearch.navigateToResult('${result.url}')">
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 5px;">
                        <div style="font-weight: bold; color: #007cba; font-size: 15px;">
                            ${result.title}
                        </div>
                        <div style="color: #999; font-size: 11px; margin-left: 10px; white-space: nowrap;">
                            ${this.getTypeIcon(result.type)}
                        </div>
                    </div>
                    <div style="color: #666; font-size: 14px; line-height: 1.4; margin-bottom: 5px;">
                        ${highlightedContent}
                    </div>
    ...
    navigateResults(direction) {
        ...
        const currentIndex = this.selectedResultIndex || 0;
    ...
    selectCurrentResult() {
        const items = document.querySelectorAll('.search-result-item');
        const index = this.selectedResultIndex || 0;
